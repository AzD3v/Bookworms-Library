...
/**
 * @property string api_url_movie
 */

class Movieworld extends CI_Controller {
    ...
    public function __construct()
    {
        ...
        // $this->api_url_movie='http://controlaltdelete.pt/uac/movie/index.php/api/movie';
        $this->api_url_movie='http://localhost/Universidade/Web-Services/Academic-Project-1/index.php/api/movie/';
        // Helpers
    ...
    {
        $this->load->view('movieworld/movies_world_welcome');
    }
    ...
    function addMovie($post_data)
    {
        ...
        $con = curl_init();
        curl_setopt($con, CURLOPT_URL, $this->api_url_movie . '/addmovie/');
    ...
    function addMovieForm()
    {
        ...
        $this->load->view('movieworld/add_movie_form');
    ...
    // Get movie method
    function getMovie($id=0)
    {
        $con = curl_init();
        curl_setopt($con, CURLOPT_URL, $this->api_url_movie. '/getmovie/'. ($id!=0 ? 'id/'.$id : ''));
        curl_setopt($con, CURLOPT_RETURNTRANSFER, true);
        $response = curl_exec($con);
        if (!curl_errno($con)) {
            switch ($http_code = curl_getinfo($con, CURLINFO_HTTP_CODE)) {
    ...
    function viewMovie($id = 0)
    {
        ...
        else
        {
            ...
            if (empty($movie))
            {
                echo "Não foi encontrado nenhum filme";
            } else
            {
                $this->load->view('geral/header');
                $this->load->view('movieworld/viewmovie', $movie[0]);
                $this->load->view('geral/footer');
            }
